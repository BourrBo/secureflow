...
const modules = [
  {
    icon: Code2,
    name: "SAST",
    desc: "Deep static analysis across 20+ languages — injection, XSS, auth flaws and unsafe data flows, 500+ detection patterns.",
    status: "Live",
  ...
  },
  {
    icon: Package,
    name: "SCA",
    desc: "Surfaces vulnerable dependencies with CVSS + exploit-likelihood scoring and auto SBOM (CycloneDX / SPDX).",
    status: "Live",
    tone: "success" as const,
  },
  {
    icon: KeyRound,
    name: "Secrets",
    desc: "800+ detection patterns for cloud keys, tokens and passwords — including your full commit history.",
    status: "Live",
    tone: "success" as const,
  },
  {
    icon: Boxes,
    name: "IaC",
    desc: "Scan Terraform, Helm, Kubernetes and CloudFormation before infrastructure is ever provisioned.",
    status: "Live",
    tone: "success" as const,
  },
  {
    icon: Container,
    name: "Container",
    desc: "Image analysis for Docker & Kubernetes — OS CVEs, misconfigurations and baked-in secrets.",
    status: "Beta",
    tone: "info" as const,
  },
  {
    icon: Bot,
    name: "AI Fix Engine",
    desc: "One-click remediation powered by Claude — context-aware, PR-ready fixes, not just suggestions.",
    status: "Soon",
...
const steps = [
  {
    ...
    desc: "Link GitHub, GitLab, Bitbucket or Azure DevOps in one click. Auto-discovers your whole org.",
  },
  {
    n: "02",
    title: "Scan on every PR",
    desc: "CI/CD plugin runs all enabled modules on push and pull request. Zero developer workflow change.",
  },
  {
    n: "03",
    title: "Fix what matters",
    desc: "CVSS + EPSS prioritization surfaces the riskiest first. Assign, triage, and merge with confidence.",
  },
];
...
const tiers = [
  {
    name: "Starter",
    price: "Free",
    tag: "For solo devs & OSS",
    features: ["3 repositories", "SAST + Secrets", "Community support", "SBOM export"],
    cta: "Start free",
  ...
  },
  {
    ...
    price: "$29",
    per: "/dev / mo",
    tag: "For growing engineering teams",
    features: [
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-6xl px-6 pt-24 pb-20 text-center md:pt-32">
          ...
          <p className="animate-fade-up mx-auto mt-6 max-w-2xl text-base text-muted-foreground md:text-lg">
            SecureFlow unifies six security scanning modules in one developer-first platform — so
            your team ships fast without leaving the door open.
          </p>
      ...
      <section id="features" className="mx-auto max-w-6xl px-6 py-24">
        <div className="max-w-2xl">
          ...
          <p className="mt-4 text-muted-foreground">
            From your first line of code to production deployment — SecureFlow has every layer
            covered by best-in-class open-source engines, unified under one dashboard.
          </p>
        </div>
      ...
      <section id="how" className="relative border-t border-border/60 bg-sidebar/40">
        <div className="mx-auto max-w-6xl px-6 py-24">
          <div className="text-center">
            ...
            <h2 className="mt-3 font-display text-4xl font-bold tracking-tight md:text-5xl">
              From commit to clean in three steps
            </h2>
          </div>
      ...
      <section id="integrations" className="mx-auto max-w-6xl px-6 py-24">
        <div className="grid gap-12 md:grid-cols-[1fr_1.2fr] md:items-center">
          <div>
            ...
            <p className="mt-4 text-muted-foreground">
              Source control, CI/CD, chat, ticketing, cloud, observability — SecureFlow slots in
              without asking your team to change how they work.
            </p>
            <div className="mt-6 flex flex-wrap gap-2 text-sm">
              {["GitHub Actions", "GitLab CI", "Jenkins", "CircleCI"].map((t) => (
                <span
                  key={t}
                  className="rounded-full border border-border bg-secondary/40 px-3 py-1 text-muted-foreground"
                >
                  {t}
                </span>
              ))}
          ...
          <div className="grid grid-cols-3 gap-3 sm:grid-cols-4">
            {integrations.map((i) => (
              <div
                key={i}
                className="surface-card grid aspect-square place-items-center rounded-xl text-center text-sm font-medium text-muted-foreground transition-colors hover:text-foreground"
              >
      ...
      <section className="mx-auto max-w-4xl px-6 pb-24">
        <div className="surface-card relative overflow-hidden rounded-3xl p-10 text-center">
          ...
          <blockquote className="mt-4 font-display text-2xl font-medium leading-snug md:text-3xl">
            “We replaced four separate security tools with SecureFlow in a weekend. Our
            mean-time-to-remediation dropped 71% in the first month.”
          </blockquote>
          <div className="mt-6 text-sm text-muted-foreground">
            <span className="font-semibold text-foreground">Priya Ramanathan</span> · VP
            Engineering, Northwind
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-6xl px-6 py-24">
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {tiers.map((t) => (
              <div
                key={t.name}
                className={`surface-card relative rounded-2xl p-8 ${t.highlight ? "ring-1 ring-primary/40 shadow-[var(--shadow-glow)]" : ""}`}
              >
                {t.highlight && (
                  <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-[image:var(--gradient-primary)] px-3 py-1 font-mono text-[10px] uppercase tracking-widest text-primary-foreground">
                    Most popular
                  </span>
                )}
        ...
        <div className="relative overflow-hidden rounded-3xl border border-border bg-[image:var(--gradient-primary)] p-12 text-center">
          ...
          <div className="relative">
            ...
            <div className="mt-8 flex flex-wrap justify-center gap-3">
              ...
              >
                <Link to="/dashboard">
                  See dashboard <ArrowRight className="h-4 w-4" />
                </Link>
              </Button>
            </div>
          </div>

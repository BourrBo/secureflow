const lines: Array<{ type: string; text?: string; parts?: Array<{ c: string; t?: string }> }> = [
  ...
  },
  {
    ...
    parts: [
      { c: "✓", t: "ok" },
      { c: " IaC                 " },
      { c: "5 misconfigs", t: "high" },
      { c: " in terraform/main.tf" },
    ],
  },
  {
    type: "row",
    parts: [{ c: "✓", t: "ok" }, { c: " Container           " }, { c: "Clean", t: "ok" }],
  },
  { type: "muted", text: "# ─────────────────────────────────────────" },
  {
    type: "row",
    parts: [
      ...
      { c: " Security score: " },
      { c: "64/100", t: "high" },
      { c: " · PR blocked · Jira tickets created" },
    ],
  },
  {
    type: "row",
    parts: [
      { c: "→", t: "info" },
      { c: " Full report: " },
      { c: "app.secureflow.io/r/ag-2025", t: "info" },
    ],
  },
];
...
const color = (t?: string) =>
  t === "ok"
    ...
    : t === "crit"
      ? "text-[oklch(0.70_0.22_20)]"
      : t === "high"
        ? "text-[oklch(0.80_0.15_75)]"
        : t === "info"
      ...
      <div className="bg-[oklch(0.12_0.02_260)] p-5 text-left font-mono text-[13px] leading-relaxed">
        {lines.map((l, i) => (
          ...
          >
            ...
            {l.text}
            {l.parts?.map((p, j) => (
              <span key={j} className={color(p.t)}>
                {p.c}
              </span>
            ))}
          </div>
